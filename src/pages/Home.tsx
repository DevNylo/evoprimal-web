...
export default function Home() {
  ...
  return (
    // TEMA VERDE RESTAURADO NA SELEÇÃO
    <div className="min-h-screen bg-transparent text-white font-sans selection:bg-green-600 selection:text-white">
      
      {/* 1. HERO SECTION */}
      {/* CORREÇÃO DE ESPAÇAMENTO: Aumentado para pt-40 (160px) para limpar totalmente a Navbar */}
        ...
        {loading && banners.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center h-[600px]">
            ...
            <div className="space-y-6 animate-fade-in-right flex flex-col justify-center h-full relative z-10">
               <div className="h-[300px] flex flex-col justify-end pb-4">
                 <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-[0.9] text-zinc-100">
                   ...
                   {/* VOLTAMOS PARA O VERDE */}
                   <span className="text-green-600 transition-all duration-500">{slides[currentSlide].highlight}</span> <br />
               ...
               <div className="h-[60px] flex items-start">
                 {/* Borda verde */}
                 <p className="text-zinc-400 text-lg border-l-4 border-green-600 pl-4 max-w-md line-clamp-2">
                   {slides[currentSlide].desc}
                 </p>
               </div>
               
               <div className="flex gap-4 pt-4 h-[80px] items-start">
                  <button className="bg-green-600 hover:bg-green-500 text-white px-8 py-4 rounded font-black uppercase tracking-widest shadow-[0_0_20px_rgba(22,163,74,0.3)] transition-all hover:-translate-y-1">
                    Ver Ofertas
                  </button>
                    ...
                    <div className="flex gap-2 items-center ml-4 mt-4">
                       {slides.map((_, idx) => (
                         <div 
                           ...
                           className={`h-2 rounded-full transition-all duration-300 cursor-pointer ${idx === currentSlide ? "w-8 bg-green-500" : "w-2 bg-zinc-700"}`}
                         ></div>
                       ))}
                    </div>
                  )}
            ...
            <div className="relative h-full w-full flex items-center justify-center overflow-hidden">
                {/* Glow Verde */}
                <div className="absolute w-[60%] h-[60%] bg-green-600/20 blur-[100px] rounded-full animate-pulse"></div>
                <img 
                  key={currentSlide} 
      ...
      <section className="py-20 border-b border-white/5 bg-black/20">
        <div className="max-w-[1280px] mx-auto px-6">
          <div className="flex items-center justify-between mb-10">
             <h2 className="text-xl font-black uppercase tracking-widest text-white border-l-4 border-green-600 pl-4">Categorias</h2>
             <div className="flex gap-2">
                <button onClick={scrollLeft} className="p-3 bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:border-green-600 rounded transition-colors"><ChevronLeft size={20}/></button>
                <button onClick={scrollRight} className="p-3 bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:border-green-600 rounded transition-colors"><ChevronRight size={20}/></button>
             </div>
          </div>
          <div ref={carouselRef} className="flex gap-6 overflow-x-auto scrollbar-hide snap-x pb-4" style={{ scrollbarWidth: 'none' }}>
            {categories.map((cat, i) => (
              <Link key={i} to={`/categoria/${cat.slug}`} className="min-w-[260px] snap-start bg-[#0a0a0a]/50 border border-white/10 hover:border-green-600/50 p-8 rounded-xl flex flex-col items-center text-center group transition-all duration-300 hover:-translate-y-2 cursor-pointer backdrop-blur-sm !no-underline">
                 ...
                 <span className="w-full mt-auto border border-zinc-800 text-zinc-400 text-[10px] font-bold uppercase py-3 rounded group-hover:bg-green-700 group-hover:text-white group-hover:border-green-700 transition-all block">Ver Opções</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-6 max-w-[1280px] mx-auto">
         <div className="mb-12 border-b border-white/5 pb-6 flex justify-between items-end">
            <div>
               <p className="text-green-600 font-bold uppercase tracking-widest text-xs mb-2">Loja Oficial</p>
